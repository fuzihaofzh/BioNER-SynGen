...
class BertForEntity(BertPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8, user_mode = {}):
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        ...
        if "onlyemb" in self.user_mode:#maple
...
class EntityModel():
    ...
    def __init__(self, args, num_ner_labels):
        ...
        self.user_mode = {e.split('=')[0] : e.split('=')[1] if len(e.split('=')) > 1 else None for e in (args.user_mode[0].split(',') if type(args.user_mode) is not str else args.user_mode.split(',')) }
        ...
        if args.use_albert:
            ...
            self.tokenizer = BertTokenizer.from_pretrained(vocab_name, user_mode = self.user_mode)
            self.bert_model = BertForEntity.from_pretrained(bert_model_name, num_ner_labels=num_ner_labels, max_span_length=args.max_span_length, user_mode = self.user_mode)

        self.bert_model.user_mode = self.user_mode
