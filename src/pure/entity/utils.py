...
def convert_dataset_to_samples(dataset, max_span_length, ner_label2id=None, context_window=0, split=0, part = "test", user_mode = {}):
    """
        ...
        for i, sent in enumerate(doc):
            ...
            for i in range(len(sent.text)):
                for j in range(i, min(len(sent.text), i+max_span_length)):
                    if "randneg" in user_mode:
                        if part == "train" and random.random() > float(user_mode['randneg']) and len(sample['spans']) > 2 and (i, j) not in sent_ner:
                            continue
                    elif "edgeneg" in user_mode:
                        isedge = any([i <= span[0] <= j or i <= span[1] <= j for span in sample['spans']])
                        if part == "train" and random.random() > float(user_mode['edgeneg']) and len(sample['spans']) > 2 and (i, j) not in sent_ner and not isedge:
                            continue
