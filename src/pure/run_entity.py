...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['ace04', 'ace05', 'scierc', "ncbi"])
    ...
    if "wm" in user_mode:
        args.data_dir = args.data_dir + "-wm"
    if "pbd" in user_mode:
        args.data_dir = args.data_dir + "-pbd"
    ...
    if 'train' in user_mode:
        args.train_data = os.path.join(args.data_dir, f"../{user_mode['train']}", 'train.json')
        args.dev_data = os.path.join(args.data_dir, f"../{user_mode['train']}", 'dev.json')
    if 'cross' in user_mode:
        cross_map = {
            "ncbi" : "bc5cdr-d",
            "bc5cdr-d" : "ncbi",
            "bc5cdr-c" : "bc4chemd",
        ...
        odata = args.data_dir.split("/")[-1]
        if "wm" in user_mode:
            cross_map = {k : cross_map[k] + "-wm" for k in cross_map}
        args.train_data = os.path.join(args.data_dir, f"../{cross_map[odata]}", 'train.json')
        args.dev_data = os.path.join(args.data_dir, f"../{cross_map[odata]}", 'dev.json')
    ...
    if 'ptm' in user_mode:
        if user_mode['ptm'] == 'our':
            model_map = {
                "ncbi" : "output/sapbert/ncbi__edge,pos=umlsdi,neg=sap,ppreg,pndreg=0.1,seed=0",
                "bc5cdr-d" : "output/sapbert/bc5cdr-d__edge,pos=umlsdi,neg=sap,ppreg,pndreg=0.1,seed=0",
                "bc5cdr-c" : "output/sapbert/bc5cdr-c__edge,pos=umlsc,neg=sapc,ppreg,pndreg=0.1,seed=0",
                "bc4chemd" : "output/sapbert/bc4chemd__edge,pos=umlsc,neg=sapc,ppreg,pndreg=0.1,seed=0",
                "s800" : "output/sapbert/s800__edge,pos=umls-scn,neg=umls-scn,ppreg,pndreg=0.1,seed=0",
                "linnaeus" : "output/sapbert/linnaeus__edge,pos=umls-scn,neg=umls-scn,ppreg,pndreg=0.1,seed=0",
            }
            odata = args.data_dir.split("/")[-1]
            args.model = model_map[odata]
    ...
    dev_samples, dev_ner = convert_dataset_to_samples(dev_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "dev", user_mode = user_mode)
    dev_batches = batchify(dev_samples, args.eval_batch_size)

    if args.do_train:
        train_data = Dataset(args.train_data)
        train_samples, train_ner = convert_dataset_to_samples(train_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "train", user_mode = user_mode)
    ...
    if args.do_eval:
        ...
        test_samples, test_ner = convert_dataset_to_samples(test_data, args.max_span_length, ner_label2id=ner_label2id, context_window=args.context_window, part = "test", user_mode = user_mode)
