...
class Sap_Metric_Learning(nn.Module):
    def __init__(self, encoder, learning_rate, weight_decay, use_cuda, pairwise, 
            loss, use_miner=True, miner_margin=0.2, type_of_triplets="all", agg_mode="cls", user_mode = {}, scmodel = None):
        ...
        if 'edge' in user_mode or 'dinner' in user_mode:
            self.optimizer = optim.AdamW([{'params': scmodel.parameters()},], 
                lr=self.learning_rate, weight_decay=self.weight_decay
            )
        #if 'fixenc' in user_mode:
        ...
        #    )
        self.user_mode = user_mode # maple
